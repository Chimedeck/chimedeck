...
describe('GET /api/v1/boards/:boardId/state-transitions', () => {
  it('returns default graph and lazily creates row when missing', async () => {
    ...
    const body = await res.json() as {
      data: { enabled: boolean; graph: { nodes: Array<{ listId: string; label: string }> } };
    };
  ...
  it('returns existing graph when transition row already exists', async () => {
    dataStore.board_state_transitions.push({
      id: 'st-1',
      board_id: 'board-1',
      enabled: true,
      graph_data: {
        nodes: [{ id: 'list-1', listId: 'list-1', label: 'Todo', positionX: 10, positionY: 20 }],
        edges: [],
        notes: [],
      },
    ...
    const body = await res.json() as { data: { enabled: boolean; graph: { nodes: Array<{ positionX: number }> } } };
    expect(body.data.enabled).toBe(true);
    expect(body.data.graph.nodes[0]?.positionX).toBe(10);
  });
  ...
  it('returns 404 when board does not exist', async () => {
    const req = new Request('http://localhost/api/v1/boards/missing-board/state-transitions', {
      method: 'GET',
      headers: { Authorization: 'Bearer test-token' },
    });

    const res = await handleGetStateTransitions(req, 'missing-board');
