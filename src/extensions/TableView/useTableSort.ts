...
function cardSortValue(row: TableRowData, column: ColumnKey): unknown {
  switch (column) {
    case 'title':
      return row.title;
    case 'list':
      return row.listTitle;
    case 'assignees':
      return row.members.map((m) => m.name ?? m.email).join(', ');
    case 'labels':
      return row.labels.map((l) => l.name).join(', ');
    case 'due_date':
      return row.due_date;
    case 'start_date':
      return row.start_date;
    case 'value':
      return row.amount != null ? parseFloat(row.amount) : null;
    default:
      return null;
  }
}

export function useTableSort({ cards, lists }: { cards: Card[]; lists: Record<string, List> }) {
  ...
  const sortedRows = useMemo(() => {
    ...
    return [...rows].sort((a, b) =>
      compareValues(cardSortValue(a, col), cardSortValue(b, col), dir)
    );
  }, [rows, sortState]);
