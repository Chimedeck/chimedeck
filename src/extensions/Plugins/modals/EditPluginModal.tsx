...
const EditPluginModal = ({ open, plugin, isSubmitting, serverError, onClose, onSubmit }: Props) => {
  ...
  const validate = (): boolean => {
    ...
    if (!description.trim())
      errs.description = translations['plugins.editModal.validation.descriptionRequired'];
    if (!connectorUrl.trim())
      errs.connectorUrl = translations['plugins.editModal.validation.connectorUrlRequired'];
    else if (!HTTPS_REGEX.test(connectorUrl))
      errs.connectorUrl = translations['plugins.editModal.validation.connectorUrlInvalid'];
    ...
    if (authorEmail && !EMAIL_REGEX.test(authorEmail))
      errs.authorEmail = translations['plugins.editModal.validation.emailInvalid'];
    if (supportEmail && !EMAIL_REGEX.test(supportEmail))
      errs.supportEmail = translations['plugins.editModal.validation.emailInvalid'];
    ...
    >
      ...
      <div className="relative w-full max-w-lg max-h-[90vh] flex flex-col bg-bg-base rounded-lg shadow-2xl mx-4 overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 flex flex-col gap-4">
          ...
          {field(
            translations['plugins.editModal.field.pluginName'],
            true,
            <input
          ...
          {field(
            translations['plugins.editModal.field.description'],
            true,
            <textarea
          ...
          {field(
            translations['plugins.editModal.field.connectorUrl'],
            true,
            <input
          ...
          {field(
            translations['plugins.editModal.field.manifestUrl'],
            false,
            <input
              className={inputCls()}
          ...
          {field(
            translations['plugins.editModal.field.iconUrl'],
            false,
          ...
          {field(
            translations['plugins.editModal.field.author'],
            true,
            <input
          ...
          {field(
            translations['plugins.editModal.field.authorEmail'],
            false,
            <input
              type="email"
          ...
          {field(
            translations['plugins.editModal.field.supportEmail'],
            false,
            <input
              type="email"
          ...
          {field(
            translations['plugins.editModal.field.categories'],
            false,
            <div>
          ...
          {field(
            translations['plugins.editModal.field.whitelistedDomains'],
            false,
            <div>
              ...
              <div className="flex flex-wrap gap-1 mb-1">
                {whitelistedDomains.map((domain) => (
                  <span
                    key={domain}
                    className="flex items-center gap-1 bg-bg-overlay text-subtle text-xs rounded px-2 py-0.5"
                  >
                    {domain}
                    <IconButton
                      icon={
        ...
        <div className="flex justify-end gap-3 px-5 py-4 border-t border-border flex-shrink-0">
          ...
          <Button variant="primary" size="sm" onClick={handleSubmit} disabled={isSubmitting}>
            {isSubmitting
              ? translations['plugins.editModal.saving']
              : translations['plugins.editModal.submit']}
          </Button>
        </div>
      </div>
    </div>,
