...
const CardButtonBuilder: FC<Props> = ({ boardId, existing, onSave, onClose }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setError('Failed to save card button. Please try again.');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    ...
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-label={existing ? 'Edit card button' : 'Create card button'}
    >
      <div className="bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl w-full max-w-md flex flex-col gap-5 p-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <h2 className="flex-1 text-base font-semibold text-slate-100">
            {existing ? 'Edit Card Button' : 'New Card Button'}
          </h2>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <input
            ...
            placeholder="e.g. Mark as reviewed"
            maxLength={80}
