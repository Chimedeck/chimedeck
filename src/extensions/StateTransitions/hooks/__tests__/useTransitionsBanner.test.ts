...
describe('useTransitionsBanner helpers', () => {
  it('builds board-scoped sessionStorage keys', () => {
    expect(getTransitionsBannerSessionKey('board-123')).toBe(
      'state-transitions-banner-dismissed-board-123'
    );
  });
  ...
  it('reads dismissal from session storage key', () => {
    const storage = {
      getItem: (key: string) =>
        key === 'state-transitions-banner-dismissed-board-123' ? '1' : null,
