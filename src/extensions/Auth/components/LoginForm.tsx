...
export default function LoginForm({ onSubmit, isLoading, apiError }: LoginFormProps) {
  ...
  const mappedApiError = apiError
    ? (API_ERROR_MAP[apiError] ?? translations.errors.loginFailed)
    : null;
        ...
        {(config.oauthGoogleEnabled || config.oauthGithubEnabled) && (
          <>
            ...
            {config.oauthGoogleEnabled && (
              <OAuthButton icon={<GoogleIcon />} label={translations.oauth.continueWithGoogle} />
            )}
            {config.oauthGithubEnabled && (
              <OAuthButton icon={<GitHubIcon />} label={translations.oauth.continueWithGitHub} />
            )}
          </>
        )}
      </div>
