...
export async function resolveCoverImageUrls<T extends CardCoverFields>(
  cards: T[]
): Promise<Array<T & CardWithResolvedCover>> {
  if (cards.length === 0) return [];

  const coverAttachmentIds = [
    ...new Set(
      cards.map((card) => card.cover_attachment_id).filter((id): id is string => Boolean(id))
    ),
  ];
  ...
  const cardsWithCover = cards.map((card) => {
    ...
    const proxyUrl =
      !isGif && attachment.thumbnail_key
...
export async function resolveCoverImageUrl<T extends CardCoverFields>(
  card: T
): Promise<T & CardWithResolvedCover> {
  const [resolved] = await resolveCoverImageUrls([card]);
  return (
    resolved ?? { ...card, cover_image_url: null, cover_aspect_ratio: null, cover_is_gif: false }
  );
}
