...
const mockDB = (
  sourceCard: { id: string; title: string; description: string; board_id: string } | undefined,
  boardCards: { id: string; title: string; description: string; board_id: string }[]
): DuplicateDB => ({
...
describe('detectDuplicates', () => {
  ...
  it('returns empty when no other cards in board', async () => {
    const db = mockDB({ id: 'c1', title: 'Test Card', description: 'A test', board_id: 'b1' }, []);
    const result = await detectDuplicates({ cardId: 'c1', intent: 'test', db });
    expect(result).toEqual([]);
  });
  ...
  it('detects duplicate by shared keywords', async () => {
    ...
    const db = mockDB(
      {
        id: 'c1',
        title: 'Build authentication system with OAuth',
      ...
      [
        {
          id: 'c2',
          title: 'Authentication with OAuth login',
      ...
      [
        {
          ...
          description: 'Adjust padding on header',
          board_id: 'b1',
        },
      ]
    );
  ...
  it('returns empty when source card has no content', async () => {
    const db = mockDB({ id: 'c1', title: 'ab', description: '', board_id: 'b1' }, [
