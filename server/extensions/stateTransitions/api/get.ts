...
import { createDefaultGraphFromLists, toGraphResponse } from '../common/serializer';
...
export async function handleGetStateTransitions(req: Request, boardId: string): Promise<Response> {
  ...
  if (row) {
    ...
    const synchronizedGraph = parsedGraph ? syncGraphWithLists(parsedGraph, activeLists) : null;
    const shouldRepair = !parsedGraph || synchronizedGraph.changed || !graphValidation.ok;
  ...
  if (!row) {
    return Response.json(
      stateTransitionError('state-transition-load-failed', {
        message: 'State transitions could not be loaded',
      }),
      { status: 500 }
    );
  }
