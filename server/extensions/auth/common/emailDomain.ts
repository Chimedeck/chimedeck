...
export const extractDomain = (email: string): string => email.split('@')[1]?.toLowerCase() ?? '';

// Returns true when the email domain is in the configured allowlist,
// OR when domain restriction is disabled entirely,
// OR when public registration is enabled (EMAIL_DOMAIN_PUBLIC_ENABLED).
export const isEmailDomainAllowed = (email: string): boolean => {
  if (env.EMAIL_DOMAIN_PUBLIC_ENABLED) return true;
