...
export async function handleCreateChatSearch(req: Request, boardId: string): Promise<Response> {
  ...
  const membershipError = await boardChatSearchApiDeps.requireWorkspaceMembership(
    workspaceReq,
    boardReq.board!.workspace_id
  );
  if (membershipError) return membershipError;

  const guestError = await boardChatSearchApiDeps.requireGuestCanViewBoardChat(
    workspaceReq,
    boardId
  );
  ...
  const query =
    typeof body.query === 'string' ? body.query : typeof body.q === 'string' ? body.q : '';
  const parsedLimit =
