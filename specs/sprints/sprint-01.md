...
services:
  ...
  localstack  # LocalStack — S3-compatible local AWS emulation (USE_LOCAL_STORAGE=true)
...
- Required vars: `DATABASE_URL`, `JWT_PRIVATE_KEY`, `JWT_PUBLIC_KEY`, `APP_PORT`
- AWS S3 vars: `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, `S3_BUCKET`, `S3_REGION`, `S3_ENDPOINT` (prod only; auto-set in local dev when `FLAG_USE_LOCAL_STORAGE=true`)
...
- Flag overrides via env: `FLAG_USE_REDIS=false`, `FLAG_USE_LOCAL_STORAGE=true`, `FLAG_VIRUS_SCAN_ENABLED=false`, etc. (see [technical-decisions.md §17](../architecture/technical-decisions.md))

### 4. Database & Knex

- Knex initialised with PostgreSQL (`pg`) provider
- `db/knexfile.ts` — reads `DATABASE_URL` from `server/config/env.ts`
- First migration: `db/migrations/0001_init.ts` (empty — establishes migration history)
- `bun run db:migrate` script (`knex migrate:latest`)
- `bun run db:rollback` script (`knex migrate:rollback`)
...
- [ ] `docker compose up` starts postgres + localstack + app with no errors (no Redis required)
